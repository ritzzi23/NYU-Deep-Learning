...
class MLP:
    ...
    def forward(self, x):
        ...
        if self.f_function == 'relu':
            a1 = torch.where(z1 > 0, z1, torch.zeros_like(z1))
    ...
    def backward(self, dJdy_hat):
        ...
        # gradients for second linear layer and activation function
...
def bce_loss(y, y_hat):
    ...
    J = -torch.mean(y * torch.log(y_hat + eps) + (1 - y) * torch.log(1 - y_hat + eps))
    dJdy_hat = -(y / (y_hat + eps) - (1 - y) / (1 - y_hat + eps)) / torch.numel(y)
